...
Java_com_alevap_CmdEntryPoint_start(JNIEnv *env, __unused jclass cls, jobjectArray args) {
    pthread_t t;
    JavaVM* vm = NULL;
    ...
    /* if (access("/data/data/com.alevap/files/usr/lib/libtermux-exec.so", F_OK) == 0 && !detectTracer()
            && !getenv("XSTARTUP_LD_PRELOAD"))
        setenv("LD_PRELOAD", "/data/data/com.alevap/files/usr/lib/libtermux-exec.so", 1); */
    ...
    if (!getenv("TMPDIR")) {
        if (access("/data/data/com.alevap/files/usr/tmp", F_OK) == 0) {
            log(INFO, "TMPDIR: Found Alev path (/data/data/com.alevap/files/usr/tmp)");
            setenv("TMPDIR", "/data/data/com.alevap/files/usr/tmp", 1);
        } else if (access("/tmp", F_OK) == 0) {
            log(INFO, "TMPDIR: Fallback to system /tmp");
            setenv("TMPDIR", "/tmp", 1);
        } else {
             log(ERROR, "TMPDIR: Critical! No suitable temp dir found. Termux fallback removed.");
        }
    }
    ...
    log(INFO, "Final TMPDIR in use: \"%s\"", getenv("TMPDIR"));
    ...
    if (!getenv("XKB_CONFIG_ROOT")) {
        ...
        // Alev case
        else if (access("/data/data/com.alevap/files/usr/share/xkeyboard-config-2", F_OK) == 0)
            setenv("XKB_CONFIG_ROOT", "/data/data/com.alevap/files/usr/share/xkeyboard-config-2", 1);
                                    if (access("/data/data/com.alevap/files/usr/share/X11/xkb", F_OK) == 0) {
                                        log(INFO, "XKB: Found internal Alev path");
                                        setenv("XKB_CONFIG_ROOT", "/data/data/com.alevap/files/usr/share/X11/xkb", 1);
                                    } else {
                                        log(ERROR, "XKB: INTERNAL PATH MISSING! Bundled assets not extracted?");
                                    }
                            
                        }

    log(INFO, "XKB_CONFIG_ROOT set to: \"%s\"", getenv("XKB_CONFIG_ROOT") ?: "NULL");
...
Java_com_alevap_CmdEntryPoint_getXConnection(JNIEnv *env, __unused jobject cls) {
    int client[2];
...
JNIEXPORT jobject JNICALL
Java_com_alevap_CmdEntryPoint_getLogcatOutput(JNIEnv *env, __unused jobject cls) {
    jclass ParcelFileDescriptorClass = (*env)->FindClass(env, "android/os/ParcelFileDescriptor");
    jmethodID adoptFd = (*env)->GetStaticMethodID(env, ParcelFileDescriptorClass, "adoptFd", "(I)Landroid/os/ParcelFileDescriptor;");
...
JNIEXPORT jboolean JNICALL
Java_com_alevap_CmdEntryPoint_connected(__unused JNIEnv *env, __unused jclass clazz) {
    return conn_fd != -1;
}

JNIEXPORT void JNICALL
Java_com_alevap_CmdEntryPoint_listenForConnections(JNIEnv *env, jobject thiz) {
