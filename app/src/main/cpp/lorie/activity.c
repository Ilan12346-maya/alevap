...
#define check(cond, fmt, ...) if ((cond)) do { __android_log_print(ANDROID_LOG_ERROR, "requestConnection", fmt, ## __VA_ARGS__); goto end; } while (0)
    ...
    __android_log_print(ANDROID_LOG_DEBUG, "requestConnection", "Attempting connection to 127.0.0.1:%d", PORT);
    ...
    if (r < 0 && errno != EINPROGRESS) {
        __android_log_print(ANDROID_LOG_ERROR, "requestConnection", "failed to connect socket: %s", strerror(errno));
        goto end;
    }
    if (r < 0 && errno == EINPROGRESS) {
        // Connection is in progress; use poll to wait for it
        struct pollfd pfd = { .fd = sock, .events = POLLOUT };
        r = poll(&pfd, 1, 50);  // Increased timeout to 50ms for better reliability
        if (!r) {
            __android_log_print(ANDROID_LOG_DEBUG, "requestConnection", "Connection timeout");
            goto end;
        }
        ...
        if (so_error == ECONNREFUSED) {
            __android_log_print(ANDROID_LOG_DEBUG, "requestConnection", "Connection refused (server not started?)");
            goto end;
        }
        check(so_error != 0, "Connection failed: %s", strerror(so_error));

        __android_log_print(ANDROID_LOG_DEBUG, "requestConnection", "Sending MAGIC handshake");
    ...
    if (r == 0) {
        __android_log_print(ANDROID_LOG_DEBUG, "requestConnection", "Immediate connection, sending MAGIC");
        check(write(sock, MAGIC, sizeof(MAGIC)) < 0, "failed to send message: %s", strerror(errno));
        sent = JNI_TRUE;
    }
...
static void nativeInit(JNIEnv *env, jobject thiz) {
    ...
    if (!Charset.self) {
        ...
        MainActivity.self = FindClassOrDie(env,  "com/alevap/MainActivity");
        MainActivity.getInstance = FindMethodOrDie(env, MainActivity.self, "getInstance", "()Lcom/alevap/MainActivity;", JNI_TRUE);
...
JNIEXPORT jint JNI_OnLoad(JavaVM *vm, __unused void *reserved) {
    ...
    jclass cls = (*env)->FindClass(env, "com/alevap/LorieView");
